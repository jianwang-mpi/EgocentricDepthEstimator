...
# from models.DCNv2.dcn_v2 import DCN as DeformConvPack
# from models.deform_conv.modules.deform_conv import DeformConvPack

class _UpProjection(nn.Sequential):

    def __init__(self, num_input_features, num_output_features, deformable=False):
        super(_UpProjection, self).__init__()
        self.deformable = deformable
        if deformable is False:
            ...
            self.conv1 = DeformConvPack(num_input_features, num_output_features,
                                      kernel_size=5, stride=1, padding=2, bias=False)
            self.bn1 = nn.BatchNorm2d(num_output_features)
            self.relu = nn.ReLU(inplace=True)
            self.conv1_2 = DeformConvPack(num_output_features, num_output_features,
                                        kernel_size=3, stride=1, padding=1, bias=False)
            self.bn1_2 = nn.BatchNorm2d(num_output_features)

            self.conv2 = DeformConvPack(num_input_features, num_output_features,
                                      kernel_size=5, stride=1, padding=2, bias=False)
            self.bn2 = nn.BatchNorm2d(num_output_features)
...
class MFF(nn.Module):
    ...
    def __init__(self, block_channel, num_features=64, deformable=False):
        super(MFF, self).__init__()

        self.up1 = _UpProjection(
            num_input_features=block_channel[0], num_output_features=16, deformable=deformable)

        self.up2 = _UpProjection(
            num_input_features=block_channel[1], num_output_features=16, deformable=deformable)

        self.up3 = _UpProjection(
            num_input_features=block_channel[2], num_output_features=16, deformable=deformable)

        self.up4 = _UpProjection(
            num_input_features=block_channel[3], num_output_features=16, deformable=deformable)
        if deformable is False:
            self.conv = nn.Conv2d(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        else:
            self.conv = DeformConvPack(num_features, num_features, kernel_size=5, stride=1, padding=2, bias=False)
        self.bn = nn.BatchNorm2d(num_features)
